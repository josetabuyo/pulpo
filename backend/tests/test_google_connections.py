...
def test_list_google_connections_sin_auth(client):
    r = client.get(f"/api/bots/{BOT_ID}/google-connections")
    assert r.status_code == 401


def test_list_google_connections_admin(client):
    r = client.get(f"/api/bots/{BOT_ID}/google-connections", headers=ADMIN)
...
def test_create_google_connection_json_invalido(client):
    r = client.post(
        f"/api/bots/{BOT_ID}/google-connections",
...
def test_create_y_delete_google_connection(client):
    ...
    # Aparece en la lista
    r2 = client.get(f"/api/bots/{BOT_ID}/google-connections", headers=ADMIN)
    ids = [c["id"] for c in r2.json()]
    assert conn_id in ids

    # Eliminar
    r3 = client.delete(f"/api/bots/{BOT_ID}/google-connections/{conn_id}", headers=ADMIN)
    assert r3.status_code == 200
    assert r3.json()["ok"] is True

    # Ya no aparece
    r4 = client.get(f"/api/bots/{BOT_ID}/google-connections", headers=ADMIN)
...
def test_delete_pulpo_default_prohibido(client):
    r = client.delete(
        f"/api/bots/{BOT_ID}/google-connections/pulpo-default",
        headers=ADMIN,
    )
...
def test_google_accounts_endpoint(client):
    r = client.get(f"/api/bots/{BOT_ID}/google-accounts", headers=ADMIN)
    assert r.status_code == 200
    data = r.json()
    assert isinstance(data, list)
