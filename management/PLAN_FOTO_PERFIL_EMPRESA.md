# Plan: Foto de perfil por bot

## Objetivo
Permitir que cada bot suba una foto de perfil (logo) que se muestre en su portal y en el dashboard de admin.

## Alcance

### Backend
- Nuevo campo `logo_url` (o `logo_path`) en la tabla `bots` de SQLite.
- Endpoint `POST /api/bot/logo` — recibe multipart/form-data con la imagen, la guarda en `data/logos/{bot_id}.{ext}` y actualiza el campo en DB.
- Endpoint `GET /api/bot/logo/{bot_id}` — sirve el archivo estático (o redirigir a `/data/logos/...` si ya están expuestos como static).
- Validación: solo imágenes (MIME jpg/png/webp), tamaño máximo ~2 MB.

### Frontend — BotPage (portal del cliente)
- En la sección de perfil/header de BotPage: mostrar la foto si existe, placeholder genérico si no.
- Botón "Cambiar foto" → input file oculto → preview inline → confirmar → POST al endpoint.

### Frontend — DashboardPage (admin)
- En la card de cada bot/bot: mostrar el logo en miniatura (32–48px) junto al nombre.
...
BotPage
  └─ input[type=file] (click en avatar)
       └─ POST /api/bot/logo  (multipart, auth Bearer)
            └─ guarda en data/logos/
            └─ UPDATE bots SET logo_path=... WHERE id=...
  └─ GET /api/bot/me  →  incluir logo_url en la respuesta
```
...
- Si la bot no tiene logo, mostrar un avatar placeholder con las iniciales del nombre.
...
Cada bot ya tiene bots de WA o Telegram activos con foto de perfil. Se puede ofrecer esa imagen como opción de carga rápida:
...
- En BotPage, si la bot tiene al menos un bot conectado (WA o TG), mostrar un botón secundario: **"Usar foto de WhatsApp"** / **"Usar foto de Telegram"** además del upload manual.
...
## Notas de UI
- El avatar es clickeable solo desde BotPage (el cliente lo gestiona).
