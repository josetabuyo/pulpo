 ...
 */
export default function ChatBanners({ ads, banners, open = true, onToggle }) {
  ...
  return (
    <div className={`pc-ads ${open ? 'pc-ads--open' : ''}`}>
      <button
        type="button"
        className="pc-ads-handle"
        onClick={onToggle}
        title={open ? 'Cerrar publicidad' : 'Abrir publicidad'}
        aria-label={open ? 'Cerrar publicidad' : 'Abrir publicidad'}
      >
        <span className={`pc-ads-handle-arrow ${open ? 'pc-ads-handle-arrow--open' : ''}`}>‹</span>
      </button>
